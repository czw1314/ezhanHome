...
class BridalIndex extends React.Component {
    ...
    goTo1(id) {
        this.props.history.push({
            pathname: '/home/bridalHome/bridalApartment', state: {
                id: id,
                key: this.state.keys
            }
        })
        ...
        getEstateMsg(params).then((res) => {
            if (res.data.code == 1) {
                ...
                this.setState({
                    ...
                    dynamicsLength: res.data.estate.estateDynamics && (res.data.estate.estateDynamics.length >= 3 ? 3 : res.data.estate.estateDynamics),
                    address: address,
                    num: l - 1,
        ...
        return (
            <div className='bridalIndex'>
                ...
                <p style={{
                    color: "rgb(51,51,51",
                    textAlign: 'left',
                    display: this.state.values.recordName ? 'block' : 'none'
                }}>备案名：{this.state.values.recordName}</p>
                                    ...
                                    item.photo.map((items, indexs) =>
                                        <li className="item" data-index={index} key={indexs}>
                                            <div className={'picture'} onClick={(e) => {
                                                this.props.history.push({
                                                    pathname: '/home/bridalHome/bridalAlbum', state: {
                                                        active: indexs,
                                                        key: index
                                                    }
                                                })
                                ...
                                <ul style={{transform: `translateX(${this.state.translateXs}px)`}}>
                                    {this.state.slideData.map((item, index) =>
                                        <li className={`${this.state.active > (item.count - item.photo.length) ? (this.state.active < (item.count + 1) ? 'active' : '') : '' } `}
                                            key={index} style={{display: item.photo[0] ? 'inline-block' : 'none'}}
                                            data-count={item.photo.length}
                                            onClick={this.goTo.bind(this, item.count, item.photo.length)}
                                ...
                                <div className={'item'}>
                                    <p>物业类型：{this.state.values && this.state.values.estatePropertyTypes.map(item => {
                                        return (<span style={{marginRight: '10px'}}
                                                      key={item.propertyType}>{item.propertyType}</span>)
                                    })}</p>
                                </div>
                                <div className={'item'}>
                        ...
                        <div className={'dynamic'}>
                            <h3>楼盘动态</h3>
                            {
                                values.estateDynamics && values.estateDynamics.map((item, index) => {
                                    return (
                                        <div className={'item'}
                                             style={{display: index < this.state.dynamicsLength ? 'block' : 'none'}}
                                             key={index}>
                                            <p><span>{item.title}</span>{item.time}</p>
                            ...
                            <Button block
                                    onClick={() => this.setState({dynamicsLength: values.estateDynamics && values.estateDynamics.length || 1})}
                                    style={{
                                        display: this.state.dynamicsLength == 3 ? 'block' : 'none',
                                        height: '50px',
                                        fontSize: '16px'
                                    }}>查看更多</Button>
                            <Button block onClick={() => this.setState({dynamicsLength: 3})} style={{
                                display: this.state.dynamicsLength > 3 ? 'block' : 'none',
                                height: '50px',
                                fontSize: '16px'
                            }}>收起</Button>
                        ...
                        <div className={'nearby'}>
                            ...
                            <Tabs defaultActiveKey="1">
                                <TabPane tab="交通" key="1">
                                    <div style={{paddingTop: '14px', paddingBottom: '14px', lineHeight: '13px'}}
                                         dangerouslySetInnerHTML={{__html: values.estateMatchings && values.estateMatchings[0] && values.estateMatchings[0].description}}>
                                    </div>
                                </TabPane>
                                <TabPane tab="医疗" key="2">
                                    <div style={{paddingTop: '14px', paddingBottom: '14px', lineHeight: '13px'}}
                                         dangerouslySetInnerHTML={{__html: values.estateMatchings && values.estateMatchings[1] && values.estateMatchings[1].description}}>
                                    </div>
                                </TabPane>
                                <TabPane tab="商业" key="3">
                                    <div style={{paddingTop: '14px', paddingBottom: '14px', lineHeight: '13px'}}
                                         dangerouslySetInnerHTML={{__html: values.estateMatchings && values.estateMatchings[2] && values.estateMatchings[2].description}}>
                                    </div>
                                </TabPane>
                                <TabPane tab="教育" key="4">
                                    <div style={{paddingTop: '14px', paddingBottom: '14px', lineHeight: '13px'}}
                                         dangerouslySetInnerHTML={{__html: values.estateMatchings && values.estateMatchings[3] && values.estateMatchings[3].description}}>
                                    </div>
                                </TabPane>
                            </Tabs>
                        </div>
                    ...
                    <div className={'right'}>
                        <div className={'first'}>
                            <div className={'titles'}>
                                <p className={'price'}>{values.referencePrice}<span
                                    style={{display: isNaN(parseInt(values.referencePrice)) ? 'none' : 'inline-block'}}>元/m²起</span>
                                </p>
                                <Rate character={<div><Icon type="heart"/><p>{this.state.star == 1 ? "取消关注" : '关注'}</p>
                                </div>} count={1} onChange={this.star.bind(this)} value={this.state.star}
                                      style={{display: (this.props.userInformation.role == 3 || this.props.userInformation.role == 4 || localStorage.getItem('role') == 4 || localStorage.getItem('role') == 3) ? "none" : 'block'}}/>
                            </div>
                        ...
                        <div className={'second'} style={{display:this.state.consultantShow.length>0?'block':'none'}}>
                            ...
                            <Pagination defaultCurrent={1} total={this.state.consultant.length} defaultPageSize={4}
                                        onChange={this.page.bind(this)} style={{
                                marginTop: '40px',
                                display: this.state.consultant.length > 4 ? 'block' : 'none'
                            }}/>
                        </div>
                    </div>
                </div>
...
export default connect(state => (
    {estateId: state.estateId, userInformation: state.userInformation}), {
    newEstateId,
    setBridalInformation
})(BridalIndex)